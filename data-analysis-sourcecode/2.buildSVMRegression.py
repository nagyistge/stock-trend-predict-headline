...
Usage: 
python 2.buildSVMRegression.py dataset.vectors dataset.labels report.csv
...
Description:  this is the second python script for the project
    ...
    By default, it run 3-fold cross-validation
"""
...
"""    
    for ker in kernels:
        ...
        clf = svm.SVC(kernel=ker, C=0.01, gamma=0.1);
        ...
        for scr in scoring_estimators:   
            try:
                ...
                scores_original = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat, dataset_labels_mat,cv=10, scoring=scr);
                
                scores_scale = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat_scaled, dataset_labels_mat,cv=10, scoring=scr);
                    
                scores_norm = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat_normal, dataset_labels_mat,cv=10, scoring=scr);
                    
                scores_binariz = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat_binarization, dataset_labels_mat,cv=10, scoring=scr);
                    
                scores_rowsoft = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat_rowsoft, dataset_labels_mat,cv=10, scoring=scr);
                    
                scores_fullsoft = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat_fullsoft, dataset_labels_mat,cv=10, scoring=scr);
                
                scores_cos = cross_validation.cross_val_score(clf, \
                    dataset_vectors_mat_cos, dataset_labels_mat,cv=10, scoring=scr);
...
for ker in kernels:
        ...
        print "== Kernel: ", ker;
        svr_reg = svm.SVR(kernel=ker, C=0.01, gamma=0.1);
        svr_reg.fit(dataset_vectors_mat,dataset_labels_mat);
        print "[ORIGINAL][",ker,"]::",svr_reg.score(dataset_vectors_mat,dataset_labels_mat);
        svr_reg = svm.SVR(kernel=ker, C=0.01, gamma=0.1);
        svr_reg.fit(dataset_vectors_mat_scaled,dataset_labels_mat);
        print "[SCALE][",ker,"]::",svr_reg.score(dataset_vectors_mat_scaled,dataset_labels_mat);
