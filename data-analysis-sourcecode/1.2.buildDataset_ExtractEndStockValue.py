...
Usage: 
   python 1.buildDataset_ExtractEndStockValue.py sem_space n_threads dataset_vector_f dataset_label_f dataset_stock_f
E.g from command line: 
"python 1.2.buildDataset_ExtractEndStockValue.py ../spaces/heads.pkl 4 dataset.vectors dataset.labels dataset.stocks"
"python 1.2.buildDataset_ExtractEndStockValue.py ../spaces/heads.pkl 4 testset.vectors testset.labels testset.stocks"
...
Parameters:
    ...
    - [OUT] dataset_stock_f: name of the dataset stock file
...
COLLECTION_NAME	 = 'crawleddata_test';	# Name of the collection
STOCK_COLLECTION_NAME='stockdata_USA300m_test' # Name of the stock data collection
...
QUERY_START_TIME = datetime(2016, 04, 01, 12, 0, 0, 0);
...
if __name__ == '__main__':
    ...
    while (current_time < datetime.now() and time_out_count < 300):
        ...
        if (stock_change>0):    # close value larger than open value
            label = +1;         # +1 -> positive
            #stock_val = close_value; # Save the latest value
            stock_val = stock_change; # Save the changed price
        ...
        """
        dataset_stocks_list.append(stock_val);
        result_count+=1;
        
        # BREAK IT WHEN DONElabel
        if (count>10000):
            break;
    ...
    # Finalize stock data
    dataset_stocks = csr_matrix(dataset_stocks_list);
...
#    dataset_label_f.close();
    dataset_stock_f = open(sys.argv[5], 'w')    
    pickle.dump(dataset_stocks, dataset_stock_f);
    dataset_stock_f.close();